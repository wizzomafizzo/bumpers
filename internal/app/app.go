...
func createDatabaseAndStateManager(ctx context.Context, projectRoot string) (
	dbManager *database.Manager, stateManager *storage.StateManager,
) {
	if projectRoot == "" {
...
func NewAppWithFileSystem(configPath, workDir string, fs afero.Fs) *App {
	...
	hookProcessor := apphooks.NewHookProcessor(configValidator, workDir, stateManager)
...
func (a *App) extractAndLogIntent(ctx context.Context, event *hooks.HookEvent) string {
	if defaultHookProcessor, ok := a.hookProcessor.(*apphooks.DefaultHookProcessor); ok {
